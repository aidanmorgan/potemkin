...
describe('Stripe example — consumer integration', () => {
  ...
  it('handles a card decline: confirming with the decline token returns a 402 card_error', async () => {
    ...
    const confirmed = await stripe.post(`/v1/payment_intents/${ok['id'] as string}/confirm`, {
      form: {},
    });
  ...
  it('retrying a create with the same Idempotency-Key does not duplicate the customer', async () => {
    ...
    const first = await stripe.post('/v1/customers', {
      form: { email: 'idem@example.com' },
      headers,
    });
    ...
    const retry = await stripe.post('/v1/customers', {
      form: { email: 'idem@example.com' },
      headers,
    });
    ...
    const list = (await stripe.get('/v1/customers', { query: { email: 'idem@example.com' } }))
      .body as Record<string, unknown>;
  ...
  it('forces the created timestamp forward with X-Potemkin-Clock-Offset', async () => {
    const baseline = (await stripe.post('/v1/customers', { form: { email: 'now@example.com' } }))
      .body as Record<string, unknown>;
