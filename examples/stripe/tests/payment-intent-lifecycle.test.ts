...
describe("Stripe example — PaymentIntent lifecycle", () => {
  ...
  it("automatic capture: confirm -> succeeded with a captured charge", async () => {
    ...
    const c = (await stripe.post(`/v1/payment_intents/${pi["id"] as string}/confirm`, { form: {} }))
      .body as Obj;
  ...
  it("manual capture: confirm -> requires_capture (uncaptured charge), capture -> succeeded", async () => {
    ...
    const confirmed = (await stripe.post(`/v1/payment_intents/${id}/confirm`, { form: {} }))
      .body as Obj;
    expect(confirmed["status"]).toBe("requires_capture");
    const before = (await stripe.get(`/v1/charges/${confirmed["latest_charge"] as string}`))
      .body as Obj;
    expect(before["captured"]).toBe(false);
    expect(before["amount_captured"]).toBe(0);

    const captured = (await stripe.post(`/v1/payment_intents/${id}/capture`, { form: {} }))
      .body as Obj;
    expect(captured["status"]).toBe("succeeded");
    const after = (await stripe.get(`/v1/charges/${captured["latest_charge"] as string}`))
      .body as Obj;
  ...
  it("cancel moves a non-terminal intent to canceled with a reason", async () => {
    ...
    const canceled = (
      await stripe.post(`/v1/payment_intents/${pi["id"] as string}/cancel`, {
  ...
  it("guards reject invalid transitions: confirm/cancel/capture on a terminal intent", async () => {
    ...
    expect(((reConfirm.body as Obj)["error"] as Obj)["code"]).toBe(
  ...
  it("cannot update a terminal intent (guarded by requires)", async () => {
    ...
    const update = await stripe.post(`/v1/payment_intents/${id}`, {
      form: { description: "too late" },
    });
