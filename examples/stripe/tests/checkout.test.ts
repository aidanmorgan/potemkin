...
describe('Stripe example — checkout + refund flow', () => {
  ...
  it('a customer buys a product: catalog -> intent -> charge -> partial refund reconciles', async () => {
    // 1. A customer.
    const customer = (
      await stripe.post('/v1/customers', { form: { email: 'buyer@example.com', name: 'Buyer' } })
    ).body as Obj;
    ...
    const price = (
      await stripe.post('/v1/prices', {
        form: { product: product['id'], currency: 'usd', unit_amount: 4200 },
      })
    ).body as Obj;
    ...
    const confirmed = (
      await stripe.post(`/v1/payment_intents/${pi['id'] as string}/confirm`, { form: {} })
    ).body as Obj;
    ...
    const refund1 = (await stripe.post('/v1/refunds', { form: { charge: chargeId, amount: 1200 } }))
      .body as Obj;
  ...
  it('a declined card never produces a charge; the consumer recovers on a new intent', async () => {
    ...
    const charges = (
      await stripe.get('/v1/charges', { query: { payment_intent: pi['id'] as string } })
    ).body as Obj;
    ...
    const ok = (
      await stripe.post('/v1/payment_intents', {
        form: { amount: 5000, currency: 'usd', payment_method: 'pm_card_visa' },
      })
    ).body as Obj;
    const confirmed = (
      await stripe.post(`/v1/payment_intents/${ok['id'] as string}/confirm`, { form: {} })
    ).body as Obj;
    expect(confirmed['status']).toBe('succeeded');
