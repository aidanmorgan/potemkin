Feature: Ingestion and Command Translation
  ...
  Scenario: REQ-12 — Invalid request payload is rejected with contract-violation
    When I send a POST to "/leads/new-id-001" with an invalid body
  ...
  Scenario: REQ-15b — Created lead persisted via correct boundary routing
    When I create a lead with name "Grace" and email "grace@example.com"
    Then the response status should be 201
    And the lead should be persisted in the state graph
