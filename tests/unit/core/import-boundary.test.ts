...
describe("core import boundary", () => {
  it("publishes parser and runtime entry points without source-specific bridges", () => {
    ...
    expect(packageJson.exports["./model/builders"]).toBeDefined();
    expect(packageJson.exports["./model/compiler"]).toBeDefined();
    expect(packageJson.exports["./model/patches"]).toBeDefined();
  });
  ...
  it("keeps the direct TypeScript compiler independent of YAML and CEL", () => {
    for (const file of ["runtimeModel.ts", "resourceModel.ts", "composition.ts"]) {
