...
const ev = (expr: string, ctx: Record<string, unknown> = {}) => cel.evaluate(expr, ctx, phase);
...
describe("CEL feature: array indexing", () => {
  ...
  it("arr[-1] should throw a range error, not silently return undefined", () => {
...
describe("CEL feature: string receiver methods", () => {
  it("string.startsWith(prefix) — receiver method dispatch", () => {
    expect(ev('"hello".startsWith("h")')).toBe(true);
  });

  it("string.endsWith(suffix) — receiver method dispatch", () => {
    expect(ev('"hello".endsWith("lo")')).toBe(true);
  });

  it("string.contains(sub) — receiver method dispatch", () => {
    expect(ev('"hello world".contains("world")')).toBe(true);
  });

  it("string.size() — receiver method dispatch", () => {
...
describe("CEL feature: has() built-in macro", () => {
  it("has(obj.field) returns true when field is present", () => {
    expect(ev("has(obj.field)", { obj: { field: "v" } })).toBe(true);
  });

  it("has(obj.missing) returns false when field is absent", () => {
...
describe("CEL feature: null-safe member access", () => {
  it("a?.b returns null when a is null, not an error (gap: null-safe access not implemented)", () => {
...
describe("CEL feature: division by zero", () => {
  it("1 / 0 throws a division-by-zero error", () => {
...
describe("CEL phase restrictions (working contract)", () => {
  ...
  it("timestamp() is banned in Reducer phase", () => {
    expect(() => cel.evaluate('timestamp("2024-01-01T00:00:00Z")', {}, CelPhase.Reducer)).toThrow(
      "CEL_PHASE_BANNED",
    );
  });
...
describe("CEL feature: comprehensions", () => {
  ...
  it("nested comprehensions", () => {
    ...
    expect(ev("[[1,2],[3,4]].map(row, row.map(x, x * 10))")).toEqual([
      [10, 20],
      [30, 40],
    ]);
  });
  ...
  it("comprehension using context variable", () => {
    expect(ev("items.filter(x, x > threshold)", { items: [1, 5, 2, 8, 3], threshold: 4 })).toEqual([
      5, 8,
    ]);
  });
});
