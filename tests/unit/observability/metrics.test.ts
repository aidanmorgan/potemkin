import {
  createEngineMetrics,
  createRuntimeOtelMetricObserver,
...
describe("observability/metrics", () => {
  ...
  it("creates one counter per runtime metric name and preserves fields", () => {
    const add = jest.fn();
    const createCounter = jest.fn(() => ({ add }));
    const observer = createRuntimeOtelMetricObserver({ createCounter } as unknown as Meter);

    observer("runtime.requests.completed", 2, { operation: "createThing" });
    observer("runtime.requests.completed", 1, { operation: "createThing" });
    observer("runtime.requests.failed", 1, { status: "503" });

    expect(createCounter).toHaveBeenCalledTimes(2);
    expect(add).toHaveBeenNthCalledWith(1, 2, { operation: "createThing" });
    expect(add).toHaveBeenNthCalledWith(2, 1, { operation: "createThing" });
    expect(add).toHaveBeenNthCalledWith(3, 1, { status: "503" });
  });
});
