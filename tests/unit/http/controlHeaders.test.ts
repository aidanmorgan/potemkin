...
describe('parseControlHeaders', () => {
  ...
  describe('Tier 1 — transparency', () => {
    it('parses dry-run as boolean across canonical forms', () => {
      for (const truthy of ['true', '1', 'yes', 'on', 'TRUE', 'YES']) {
        expect(parseControlHeaders({ 'x-potemkin-dry-run': truthy }).transparency.dryRun).toBe(
          true,
        );
      }
      for (const falsy of ['false', '0', 'no', 'off']) {
        expect(parseControlHeaders({ 'x-potemkin-dry-run': falsy }).transparency.dryRun).toBe(
          false,
        );
      }
    });
    ...
    it('parses clock-offset as signed integer', () => {
      expect(
        parseControlHeaders({ 'x-potemkin-clock-offset': '3600000' }).transparency.clockOffsetMs,
      ).toBe(3600000);
      expect(
        parseControlHeaders({ 'x-potemkin-clock-offset': '-500' }).transparency.clockOffsetMs,
      ).toBe(-500);
    });

    it('rejects non-integer clock-offset', () => {
      expect(
        parseControlHeaders({ 'x-potemkin-clock-offset': '3.5' }).transparency.clockOffsetMs,
      ).toBeUndefined();
      expect(
        parseControlHeaders({ 'x-potemkin-clock-offset': 'banana' }).transparency.clockOffsetMs,
      ).toBeUndefined();
    });
  });
  ...
  describe('Tier 2 — side effects', () => {
    it('parses all skip-* flags as booleans', () => {
      ...
      expect(c.sideEffects.skipReactions).toBe(true);
      expect(c.sideEffects.skipDispatch).toBe(true);
    });

    it('parses max-cascade-depth as non-negative integer', () => {
      expect(
        parseControlHeaders({ 'x-potemkin-max-cascade-depth': '3' }).sideEffects.maxCascadeDepth,
      ).toBe(3);
      expect(
        parseControlHeaders({ 'x-potemkin-max-cascade-depth': '0' }).sideEffects.maxCascadeDepth,
      ).toBe(0);
      expect(
        parseControlHeaders({ 'x-potemkin-max-cascade-depth': '-1' }).sideEffects.maxCascadeDepth,
      ).toBeUndefined();
    });

    it('parses bulk-transactional flag', () => {
      expect(
        parseControlHeaders({ 'x-potemkin-bulk-transactional': 'true' }).sideEffects
          .bulkTransactional,
      ).toBe(true);
    });
  });
  ...
  describe('Tier 4 — time travel', () => {
    it('parses read-at-version as non-negative integer', () => {
      expect(
        parseControlHeaders({ 'x-potemkin-read-at-version': '5' }).timeTravel.readAtVersion,
      ).toBe(5);
      expect(
        parseControlHeaders({ 'x-potemkin-read-at-version': '0' }).timeTravel.readAtVersion,
      ).toBe(0);
    });
  ...
  describe('Tier 5 — format', () => {
    it('accepts only valid response-format values', () => {
      expect(
        parseControlHeaders({ 'x-potemkin-response-format': 'hal' }).format.responseFormat,
      ).toBe('hal');
      expect(
        parseControlHeaders({ 'x-potemkin-response-format': 'jsonapi' }).format.responseFormat,
      ).toBe('jsonapi');
      expect(
        parseControlHeaders({ 'x-potemkin-response-format': 'plain' }).format.responseFormat,
      ).toBe('plain');
      expect(
        parseControlHeaders({ 'x-potemkin-response-format': 'xml' }).format.responseFormat,
      ).toBeUndefined();
    });

    it('accepts only valid pagination-style values', () => {
      expect(
        parseControlHeaders({ 'x-potemkin-pagination-style': 'envelope' }).format.paginationStyle,
      ).toBe('envelope');
      expect(
        parseControlHeaders({ 'x-potemkin-pagination-style': 'raw' }).format.paginationStyle,
      ).toBe('raw');
      expect(
        parseControlHeaders({ 'x-potemkin-pagination-style': 'link-header' }).format
          .paginationStyle,
      ).toBe('link-header');
      expect(
        parseControlHeaders({ 'x-potemkin-pagination-style': 'weird' }).format.paginationStyle,
      ).toBeUndefined();
    });

    it('parses mask as CSV array of field names', () => {
      expect(
        parseControlHeaders({ 'x-potemkin-mask': 'email,phone,ssn' }).format.maskFields,
      ).toEqual(['email', 'phone', 'ssn']);
      expect(
        parseControlHeaders({ 'x-potemkin-mask': ' email , phone ' }).format.maskFields,
      ).toEqual(['email', 'phone']);
    });
  });
  ...
  describe('Tier 6 — observability', () => {
    ...
    it('accepts only valid log levels', () => {
      for (const lvl of ['debug', 'info', 'warn', 'error']) {
        expect(parseControlHeaders({ 'x-potemkin-log-level': lvl }).observability.logLevel).toBe(
          lvl,
        );
      }
      expect(
        parseControlHeaders({ 'x-potemkin-log-level': 'verbose' }).observability.logLevel,
      ).toBeUndefined();
    });
    ...
    it('rejects malformed metric-tag', () => {
      expect(
        parseControlHeaders({ 'x-potemkin-metric-tag': 'no-equals' }).observability.metricTag,
      ).toBeUndefined();
      expect(
        parseControlHeaders({ 'x-potemkin-metric-tag': '=no-key' }).observability.metricTag,
      ).toBeUndefined();
    });
  });
...
describe('Potemkin control aliases', () => {
  it('expands skip_reactions to the canonical request header', () => {
    expect(expandPotemkinAliases({ skip_reactions: 'true' })).toEqual({
      headers: { 'x-potemkin-skip-reactions': 'true' },
      unknown: [],
    });
  });
});
...
describe('applyMask', () => {
  it('replaces top-level fields with [MASKED]', () => {
    const masked = applyMask({ name: 'Alice', email: 'a@b.com', phone: '555-0100' }, [
      'email',
      'phone',
    ]);
  ...
  it('recurses into arrays', () => {
    const masked = applyMask([{ email: 'a@b.com' }, { email: 'c@d.com' }], ['email']);
    expect(masked).toEqual([{ email: '[MASKED]' }, { email: '[MASKED]' }]);
  });

  it('recurses into nested objects', () => {
    const masked = applyMask({ items: [{ email: 'a@b.com' }, { email: 'c@d.com' }] }, ['email']);
