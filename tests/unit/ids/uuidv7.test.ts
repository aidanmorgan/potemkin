...
describe("ids/uuidv7", () => {
  ...
  describe("createDeterministicUuidv7Source", () => {
    it("shares one monotonic, reproducible stream across consumers", () => {
      const first = createDeterministicUuidv7Source();
      const second = createDeterministicUuidv7Source();
      const firstRun = Array.from({ length: 4 }, () => first());
      const secondRun = Array.from({ length: 4 }, () => second());

      expect(secondRun).toEqual(firstRun);
      expect(new Set(firstRun).size).toBe(firstRun.length);
      expect(firstRun.every((id) => isUuidv7(id))).toBe(true);
    });
  });
