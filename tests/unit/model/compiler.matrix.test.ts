...
const compile = (definition: RuntimeDefinition, options?: { allowExternalReferences?: boolean }) =>
  compileRuntime(definition, dependencies, options);
...
describe("runtime model compiler validation", () => {
  it("indexes, freezes, and exposes valid source-neutral definitions", () => {
    const model = compile({
      ...
      helpers: [{ name: "slug", invoke: () => "orders" }],
      policies: {},
    });
  ...
  it("rejects duplicate boundary names and contract paths", () => {
    ...
    expect(() =>
      compile({
        ...
        helpers: [
          { name: "slug", invoke: () => "orders" },
          { name: "slug", invoke: () => "users" },
        ],
      }),
  ...
  it("validates dispatch targets and permits explicitly external references", () => {
