...
describe('schema/typeCheck — permutations', () => {
  ...
  describe('isAssignable — any kind', () => {
    it.each([null, 'str', 42, true, [], {}] as any[])('any accepts %j', (val) => {
  ...
  describe('isAssignable — object kind', () => {
    ...
    it('object missing required field → false', () => {
      expect(isAssignable({}, objSchema({ name: strSchema() }, { required: ['name'] }))).toBe(
        false,
      );
    });
    ...
    it('additionalProperties:undefined → extra keys not allowed', () => {
      expect(isAssignable({ name: 'x', extra: 'y' }, objSchema({ name: strSchema() }))).toBe(false);
    });
  });
  ...
  describe('validateEntityAgainstSchema', () => {
    ...
    it('null root value → ok: false (not nullable)', async () => {
      const result = await validateEntityAgainstSchema(null as any, strSchema());
      expect(result.ok).toBe(false);
    });

    it('null root value on nullable schema → ok: true', async () => {
      const result = await validateEntityAgainstSchema(null as any, strSchema({ nullable: true }));
      expect(result.ok).toBe(true);
    });

    it('null root with null kind → ok: true', async () => {
      const result = await validateEntityAgainstSchema(null as any, nullSchema());
      expect(result.ok).toBe(true);
    });
