...
const ROOT: ObjectGraphSchema = objectSchema("Root", {
  ...
  status: unionSchema("Root.status", [stringSchema("Root.status[0]"), intSchema("Root.status[1]")]),
  strict: objectSchema(
    "Root.strict",
    {
...
describe("schema/pathResolver — permutations", () => {
  ...
  describe("union schema resolution", () => {
    ...
    it("returns null when no union member has the path", () => {
      const schema = unionSchema("U", [objectSchema("U[0]", { x: stringSchema("U[0].x") })]);
