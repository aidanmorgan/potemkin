...
describe('cel/builtins', () => {
  ...
  describe('callBuiltin', () => {
    it('throws for unknown function name', () => {
      expect(() => callBuiltin('$unknown', [], { phase: CelPhase.Behavior })).toThrow(
        'CEL_UNKNOWN_BUILTIN',
      );
    });
    ...
    it('throws for $uuidv7 in Reducer phase', () => {
      expect(() => callBuiltin('$uuidv7', [], { phase: CelPhase.Reducer })).toThrow(
        'CEL_PHASE_BANNED',
      );
    });

    it('throws for $now in Reducer phase', () => {
      expect(() => callBuiltin('$now', [], { phase: CelPhase.Reducer })).toThrow(
        'CEL_PHASE_BANNED',
      );
    });
    ...
    it('phase error message includes function name and phase', () => {
      expect(() => callBuiltin('$now', [], { phase: CelPhase.Reducer })).toThrow(CelPhase.Reducer);
    });
  });
  ...
  describe('duration() rejects degenerate ISO strings', () => {
    it('duration("P") throws the unparseable-duration error', () => {
      expect(() => BUILTINS['duration']!('P')).toThrow(
        'CEL_RUNTIME_ERROR: invalid duration string',
      );
    });

    it('duration("PT") throws the unparseable-duration error', () => {
      expect(() => BUILTINS['duration']!('PT')).toThrow(
        'CEL_RUNTIME_ERROR: invalid duration string',
      );
    });
