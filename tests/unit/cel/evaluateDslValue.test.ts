...
describe("CelEvaluator.evaluateDslValue", () => {
  ...
  describe("string interpolation", () => {
    it("interpolates ${expr} within a string", () => {
      expect(cel.evaluateDslValue("Status: ${state.status}", ctx, CelPhase.Behavior)).toBe(
        "Status: NEW",
      );
    });

    it("interpolates multiple expressions", () => {
      expect(cel.evaluateDslValue("${state.status} (${state.score})", ctx, CelPhase.Behavior)).toBe(
        "NEW (50)",
      );
    });
  });
  ...
  describe("escape with $${", () => {
    it("$${...} produces literal ${...}", () => {
      expect(cel.evaluateDslValue("$${not-an-expression}", ctx, CelPhase.Behavior)).toBe(
        "${not-an-expression}",
      );
    });
  });

  describe("bare strings evaluated as CEL", () => {
