...
describe('cel/builtins — $fake, $fakeSeed, $fakeFromFormat (per-instance RNG)', () => {
  describe('$fake', () => {
    ...
    it('throws for non-string argument', () => {
      expect(() => callBuiltin('$fake', [42], fakeCtx(CelPhase.Behavior))).toThrow(
        'CEL_TYPE_ERROR',
      );
    });

    it('throws for missing dot separator', () => {
      expect(() => callBuiltin('$fake', ['noDot'], fakeCtx(CelPhase.Behavior))).toThrow(
        'module.method',
      );
    });
    ...
    it('throws for unknown faker module', () => {
      expect(() =>
        callBuiltin('$fake', ['nonexistent.method'], fakeCtx(CelPhase.Behavior)),
      ).toThrow('unknown faker category');
    });

    it('throws for unknown faker method', () => {
      expect(() =>
        callBuiltin('$fake', ['person.nonexistentMethod'], fakeCtx(CelPhase.Behavior)),
      ).toThrow('unknown faker category');
    });
  });
  ...
  describe('$fakeSeed', () => {
    ...
    it('throws for non-number argument', () => {
      expect(() => callBuiltin('$fakeSeed', ['abc'], fakeCtx(CelPhase.Behavior))).toThrow(
        'CEL_TYPE_ERROR',
      );
    });
  ...
  describe('$fakeFromFormat', () => {
    ...
    it('throws for non-string argument', () => {
      expect(() => callBuiltin('$fakeFromFormat', [42], fakeCtx(CelPhase.Behavior))).toThrow(
        'CEL_TYPE_ERROR',
      );
    });
  ...
  describe('phase restrictions', () => {
    ...
    it('bans $fake in Reducer phase', () => {
      expect(() => callBuiltin('$fake', ['person.firstName'], fakeCtx(CelPhase.Reducer))).toThrow(
        'CEL_PHASE_BANNED',
      );
    });
    ...
    it('bans $fakeSeed in Reducer phase', () => {
      expect(() => callBuiltin('$fakeSeed', [42], fakeCtx(CelPhase.Reducer))).toThrow(
        'CEL_PHASE_BANNED',
      );
    });
    ...
    it('bans $fakeFromFormat in Reducer phase', () => {
      expect(() => callBuiltin('$fakeFromFormat', ['email'], fakeCtx(CelPhase.Reducer))).toThrow(
        'CEL_PHASE_BANNED',
      );
    });
  });
  ...
  describe('CEL evaluator integration', () => {
    ...
    it('evaluates $fakeFromFormat via CEL expression string', () => {
      const cel = createCelEvaluator();
      const result = cel.evaluate(
        "$fakeFromFormat('email')",
        {},
    ...
    it('seeds $uuidv7 deterministically without changing the unseeded UUID source', () => {
      const first = createCelEvaluator().withRequestContext({ seed: 'tenant-abc' });
      const second = createCelEvaluator().withRequestContext({ seed: 'tenant-abc' });
      const different = createCelEvaluator().withRequestContext({ seed: 'tenant-def' });

      const firstValues = [
        first.evaluate('$uuidv7()', {}, CelPhase.Identity),
        first.evaluate('$uuidv7()', {}, CelPhase.Identity),
      ];
      const secondValues = [
        second.evaluate('$uuidv7()', {}, CelPhase.Identity),
        second.evaluate('$uuidv7()', {}, CelPhase.Identity),
      ];

      expect(firstValues).toEqual(secondValues);
      expect(firstValues[0]).not.toBe(firstValues[1]);
      expect(different.evaluate('$uuidv7()', {}, CelPhase.Identity)).not.toBe(firstValues[0]);
      expect(firstValues[0]).toMatch(
    ...
    it('rejects $fake in Reducer phase via CEL evaluator', () => {
      const cel = createCelEvaluator();
      expect(() => cel.evaluate("$fake('person.firstName')", {}, CelPhase.Reducer)).toThrow(
        'CEL_PHASE_BANNED',
      );
    });
  });
