...
const baseResource = {
  resource: 'customer',
  schema: 'customer',
  identity: { creation: { generate: "$concat('cus_', $uuidv7())" }, key: { from: 'path' } },
...
describe('expandResourceModules', () => {
  it('expands a resource into one boundary per contract path, sharing the schema', () => {
    ...
    expect(coll['identity']).toEqual({
      creation: { generate: "$concat('cus_', $uuidv7())" },
    });
    expect(coll['behaviors']).toEqual([
      {
        name: 'PostCustomers',
        match: { operationId: 'PostCustomers', method: 'POST', condition: 'true' },
    ...
    expect(byId['behaviors']).toEqual([
      {
        ...
        match: { operationId: 'PostCustomersCustomer', method: 'POST', condition: 'true' },
        emit: 'CustomerUpdated',
      },
      {
        name: 'DeleteCustomersCustomer',
        match: { operationId: 'DeleteCustomersCustomer', method: 'DELETE', condition: 'true' },
  ...
  it('requires emit on non-query operations and forbids it on queries', () => {
    ...
    const queryEmit = {
      ...baseResource,
      operations: [{ op: 'GetCustomers', query: true, emit: 'X' }],
    };
    expect(() => expandResourceModules(resourceModule(queryEmit), openapi)).toThrow(
      /must not declare "emit"/,
    );
  });
  ...
  it('rejects unknown resource keys (no silent drop)', () => {
    expect(() =>
      expandResourceModules(resourceModule({ ...baseResource, computd: [] }), openapi),
    ).toThrow(/unknown resource key "computd"/);
  });

  it('rejects unknown operation keys', () => {
    const bad = {
      ...baseResource,
      operations: [{ op: 'PostCustomers', emit: 'CustomerCreated', conditon: 'x' }],
    };
    expect(() => expandResourceModules(resourceModule(bad), openapi)).toThrow(
      /unknown key "conditon"/,
    );
  });
