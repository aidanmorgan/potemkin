  ...
  it('merges declared internal/computed into the schema', () => {
    const input: BoundaryInferenceInput = {
      ...
      reducers: [
        {
          on: 'LineItemAdded',
          patches: [{ op: 'append', path: '/lineItems', value: { qty: 'event.payload.qty' } }],
        },
      ],
      state: {
        computed: [
        ...
        internal: [{ name: 'audit', type: { kind: 'object', confidence: 'known', fields: {} } }],
      },
    };
    const result = buildInferredSchema(input);
...
describe('Schema-inference fixed-point cap', () => {
  ...
  it('mutually-cyclic event-payload template references terminate at a stable schema', () => {
    ...
    const result = buildInferredSchema({
      boundary: 'Cyclic',
      events: [{ name: 'E', template: { a: 'event.payload.b', b: 'event.payload.a' } }],
      reducers: [
        {
          on: 'E',
          patches: [
    ...
    expectBootCode(
      () =>
        buildInferredSchema({
          ...
          reducers: [],
          state: {
            computed: [{ name: 'a', formula: 'state.a + 1', dependsOn: ['a'] }],
          },
        }),
...
describe('Reducer cannot write computed paths (REQ-PATCH-005)', () => {
  const baseInput = (extraReducerPatch: {
    op: 'replace' | 'remove';
    path: string;
    ...
    state: {
      computed: [{ name: 'summary', formula: "'x'", dependsOn: [] }],
    },
  });
    ...
    expectBootCode(
      () =>
        buildInferredSchema({
          ...
          reducers: [{ on: 'E', patches: [{ op: 'replace', path: '/status', value: "'NEW'" }] }],
          state: {
  ...
  it('respects topological order', () => {
    ...
    const ev = {
      evaluate(formula: string, ctx: { state: Record<string, unknown> }): unknown {
        calls.push(formula);
        if (formula === 'state.a + state.b')
          return (ctx.state['a'] as number) + (ctx.state['b'] as number);
