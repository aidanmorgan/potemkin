...
import type { Patch } from '../../../src/contracts/value.js';
...
describe('patches.applyPatches — RFC 6902 ops', () => {
  ...
  it('replace overwrites an existing property', () => {
    const state = { a: 1, b: 2 };
    const { newState } = applyPatches(state, [{ op: 'replace', path: '/a', value: 99 }]);
    expect(newState).toEqual({ a: 99, b: 2 });
  });

  it('replace fails when target is missing', () => {
    expect(() => applyPatches({ a: 1 }, [{ op: 'replace', path: '/missing', value: 0 }])).toThrow(
      PatchApplyError,
    );
  });
  ...
  it('remove fails when target is missing', () => {
    expect(() => applyPatches({}, [{ op: 'remove', path: '/x' }])).toThrow(PatchApplyError);
  });

  it('move relocates a value', () => {
    const state = { a: { x: 1 }, b: {} };
    const { newState } = applyPatches(state, [{ op: 'move', from: '/a/x', path: '/b/x' }]);
    expect(newState).toEqual({ a: {}, b: { x: 1 } });
  });

  it('copy duplicates a value', () => {
    const state = { a: 1 };
    const { newState } = applyPatches(state, [{ op: 'copy', from: '/a', path: '/b' }]);
  ...
  it("add at '/items/-' appends to end", () => {
    ...
    const { newState } = applyPatches(state, [{ op: 'add', path: '/items/-', value: 3 }]);
...
describe('patches.applyPatches — Potemkin extensions', () => {
  it('append pushes to an array', () => {
    const state = { items: [1, 2] };
    const { newState } = applyPatches(state, [{ op: 'append', path: '/items', value: 3 }]);
    expect(newState).toEqual({ items: [1, 2, 3] });
  });

  it('prepend inserts at front', () => {
    const state = { items: [2, 3] };
    const { newState } = applyPatches(state, [{ op: 'prepend', path: '/items', value: 1 }]);
    expect(newState).toEqual({ items: [1, 2, 3] });
  });

  it('append fails when target is not an array', () => {
    expect(() => applyPatches({ x: 1 }, [{ op: 'append', path: '/x', value: 1 }])).toThrow(
      PatchApplyError,
    );
  });
  ...
  it('increment adds to a numeric field', () => {
    const state = { count: 5 };
    const { newState } = applyPatches(state, [{ op: 'increment', path: '/count', by: 3 }]);
    expect(newState).toEqual({ count: 8 });
  });

  it('increment fails on non-numeric target', () => {
    expect(() => applyPatches({ x: 'str' }, [{ op: 'increment', path: '/x', by: 1 }])).toThrow(
      PatchApplyError,
    );
  });
...
describe('patches.applyPatches — journal (REQ-PATCH-004)', () => {
  ...
  it('journal preserves op-specific fields', () => {
    const { journal } = applyPatches({ a: 1 }, [{ op: 'increment', path: '/a', by: 5 }]);
