...
describe("validateWorkflowForward", () => {
  ...
  it("throws when an id entry is missing extract or use", () => {
    expect(() => validateWorkflowForward({ ids: { x: { extract: "a" } } })).toThrow(
      /workflow.ids.x.use/,
    );
  });
...
describe("validateGovernanceForward", () => {
  ...
  it("rejects malformed report fields instead of forwarding them unchecked", () => {
    expect(() => validateGovernanceForward({ report: { format: 42 } })).toThrow(/report.format/);
    expect(() =>
      validateGovernanceForward({ report: { successCriteria: { minCoverage: "80" } } }),
    ).toThrow(/minCoverage/);
    expect(() =>
      validateGovernanceForward({ report: { successCriteria: { excludedEndpoints: ["/ok", 2] } } }),
...
describe("translateOverlayPatches", () => {
  ...
  it("rejects move because the source value is unavailable without the spec", () => {
    expect(() => translateOverlayPatches([{ op: "move", from: "/a/b", path: "/c/d" }])).toThrow(
      /cannot be translated without the source spec/,
    );
  });

  it("rejects copy because the source value is unavailable without the spec", () => {
    expect(() => translateOverlayPatches([{ op: "copy", from: "/a/b", path: "/c/d" }])).toThrow(
      /cannot be translated without the source spec/,
    );
  });

  it("rejects Potemkin extension ops", () => {
    expect(() => translateOverlayPatches([{ op: "increment", path: "/x", by: 1 }])).toThrow(
      /Overlay translation/,
    );
  });
});
...
describe("mergeForwardBlock (REQ-FWD-001 precedence)", () => {
  it("scalars from potemkin replace specmatic scalars", () => {
    expect(mergeForwardBlock({ name: "spec", timeout: 1 }, { timeout: 5 })).toEqual({
      name: "spec",
      timeout: 5,
    });
  });
  ...
  it("lists concatenate (specmatic first, potemkin after)", () => {
    expect(mergeForwardBlock({ items: ["a", "b"] }, { items: ["c"] })).toEqual({
      items: ["a", "b", "c"],
    });
  });
