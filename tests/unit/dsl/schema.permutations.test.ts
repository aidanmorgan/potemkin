...
describe("dsl/schema — permutations", () => {
  ...
  describe("root required fields", () => {
    ...
    it("throws when boundary is empty string", () => {
      expect(() => validateBoundaryConfig({ boundary: "", contract_path: "/test" })).toThrow(
        BootError,
      );
    });
  ...
  describe("fallback_override field", () => {
    ...
    it("throws when fallback_override is a string", () => {
      expect(() => validateBoundaryConfig(minimalValid({ fallback_override: "yes" }))).toThrow(
        BootError,
      );
    });

    it("throws when fallback_override is a number", () => {
      expect(() => validateBoundaryConfig(minimalValid({ fallback_override: 1 }))).toThrow(
        BootError,
      );
    });
  ...
  describe("identity config", () => {
    ...
    it("throws when identity is not an object", () => {
      expect(() => validateBoundaryConfig(minimalValid({ identity: "bad" }))).toThrow(BootError);
    });

    it("throws when identity.creation is not an object", () => {
      expect(() => validateBoundaryConfig(minimalValid({ identity: { creation: "bad" } }))).toThrow(
        BootError,
      );
    });
  ...
  describe("behaviors array", () => {
    ...
    it("throws when behaviors is not an array", () => {
      expect(() => validateBoundaryConfig(minimalValid({ behaviors: "not-array" }))).toThrow(
        BootError,
      );
    });

    it("throws when a behavior is not an object", () => {
      expect(() => validateBoundaryConfig(minimalValid({ behaviors: ["bad"] }))).toThrow(BootError);
    });
    ...
    it("throws when behavior name is missing", () => {
      expect(() =>
        validateBoundaryConfig(
          minimalValid({
            behaviors: [
              { match: { operationId: "createThing", condition: "true" }, emit: "Created" },
            ],
          }),
        ),
      ).toThrow(BootError);
    ...
    it.each(["createB", "updateB", "getB"])('accepts any operationId "%s"', (operationId) => {
      const cfg = {
        boundary: "B",
        contract_path: "/b",
  ...
  describe("reducers array", () => {
    it("throws when reducers is not an array", () => {
      expect(() => validateBoundaryConfig(minimalValid({ reducers: "bad" }))).toThrow(BootError);
    });
    ...
    it("accepts reducer with a replace patch list", () => {
      ...
      expect(result.reducers[0]?.patches?.[0]).toEqual({
        op: "replace",
        path: "/status",
  ...
  describe("event_catalog", () => {
    it("throws when event_catalog is not an array", () => {
      expect(() => validateBoundaryConfig(minimalValid({ event_catalog: "bad" }))).toThrow(
        BootError,
      );
    });
  ...
  describe("initialization field", () => {
    ...
    it("throws when initialization is not an array", () => {
      expect(() => validateBoundaryConfig(minimalValid({ initialization: "bad" }))).toThrow(
        BootError,
      );
    });
  ...
  describe("query_mapping field", () => {
    ...
    it("throws when query_mapping is not an object", () => {
      expect(() => validateBoundaryConfig(minimalValid({ query_mapping: "bad" }))).toThrow(
        BootError,
      );
    });

    it("throws when query_mapping value is not a string", () => {
      expect(() => validateBoundaryConfig(minimalValid({ query_mapping: { field: 123 } }))).toThrow(
        BootError,
      );
    });
  });
