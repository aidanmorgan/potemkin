...
const minimalValid = {
  ...
  reducers: [
    { on: "Created", patches: [{ op: "replace", path: "/status", value: '${"active"}' }] },
  ],
  ...
  describe("validateBoundaryConfig", () => {
    ...
    it("throws BootError when behavior.match.intent is invalid", () => {
      const raw = {
        ...minimalValid,
        behaviors: [
          { name: "b", match: { intent: "invalid", condition: "true" }, emit: "Created" },
        ],
      };
      expect(() => validateBoundaryConfig(raw)).toThrow(BootError);
    });
      ...
      it("accepts op:upsert with key present", () => {
        expect(() =>
          validateBoundaryConfig(
            makePatch({ op: "upsert", path: "/items", key: "id", value: { id: "1" } }),
          ),
        ).not.toThrow();
      });
    ...
    describe("audit_fields", () => {
      ...
      it("throws BootError when audit_fields is not a boolean", () => {
        expect(() => validateBoundaryConfig({ ...minimalValid, audit_fields: "yes" })).toThrow(
          BootError,
        );
      });
    });
    ...
    describe("boundary-level fault_rules", () => {
      ...
      it("throws BootError when fault_rules is not an array", () => {
        expect(() => validateBoundaryConfig({ ...minimalValid, fault_rules: {} })).toThrow(
          BootError,
        );
      });
    });
    ...
    describe("behavior match.headers parsing", () => {
      ...
      it("parses multiple headers in match.headers", () => {
        ...
        expect(config.behaviors[0]?.match.headers).toEqual({
          "x-header-a": "alpha",
          "x-header-b": "beta",
        });
      });
    ...
    describe("deprecated.date validation", () => {
      ...
      it("accepts a valid ISO-8601 deprecated.date without throwing", () => {
        expect(() =>
          validateBoundaryConfig({
            ...minimalValid,
            deprecated: { date: "2025-01-01T00:00:00.000Z" },
          }),
        ).not.toThrow();
      });
    });
