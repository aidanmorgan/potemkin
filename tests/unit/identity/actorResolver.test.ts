...
describe('resolveActor — auth-mode-aware actor resolution', () => {
  describe('simple mode / no auth config — bearer shortcut', () => {
    it('parses the "Bearer <id>:<scopes>" shortcut when mode is simple', () => {
      const actor = resolveActor('Bearer mgr1:manager,admin', { mode: 'simple' });
      expect(actor).toEqual({ id: 'mgr1', scopes: ['manager', 'admin'] });
    });

    it('parses the shortcut when no auth config is present (default)', () => {
  ...
  describe('jwt mode', () => {
    ...
    it('rejects the simple "Bearer <id>:<scopes>" shortcut with a JwtValidationError', () => {
