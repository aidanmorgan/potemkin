 ...
 *  - loadCrmFixture() resolves without throwing.
...
import { loadCrmFixture, loadCrmFixtureWithGlobal } from '../../fixtures/index.js';
import { bootSystem } from '../../../src/engine/boot.js';
import { compileDsl } from '../../../src/dsl/parser.js';

describe('CRM Fixture Boot', () => {
  it('loadCrmFixture() resolves without throwing', async () => {
    await expect(loadCrmFixture()).resolves.not.toThrow();
  });
  ...
  it('loads global config with sagas, idempotency, and derived projections', async () => {
    const { dslModules, globalYaml } = await loadCrmFixtureWithGlobal();
