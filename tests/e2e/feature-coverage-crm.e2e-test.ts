...
//   direct internal engine transport invocation
//   plugin: /_potemkin/health alongside the plain /health route
...
describe("Full feature coverage walkthrough (CRM fixture)", () => {
  let app: E2eApp;
  ...
  it("plugin exposes /_potemkin/health and /health on the same port", async () => {
  ...
  it("GET /leads returns array; GET /leads?limit=N returns pagination envelope", async () => {
    const raw = await requestThroughSpecmatic(app.stubUrl, "GET", "/leads");
    expect(raw.status).toBe(200);
    expect(Array.isArray(raw.body)).toBe(true);

    const envelope = await requestThroughSpecmatic(
      app.stubUrl,
      "GET",
      "/leads",
      null,
  ...
  it("GET /leads/{id} returns the projected entity with patches: reducer state", async () => {
    ...
    const leadId = (created.body as { id: string }).id;
    const fetched = await requestThroughSpecmatic(app.stubUrl, "GET", `/leads/${leadId}`);
  ...
  it("lead lifecycle: contact → call → qualify → assertions on state at every step", async () => {
    ...
    const qualified = await requestThroughSpecmatic(
      app.stubUrl,
      "POST",
  ...
  it("cross-boundary dispatch: lead-convert → opportunity-create (LeadConversionSaga)", async () => {
    ...
    const converted = await requestThroughSpecmatic(
      app.stubUrl,
      "POST",
  ...
  it("Idempotency-Key replay returns identical body without re-creating", async () => {
    ...
    const first = await requestThroughSpecmatic(app.stubUrl, "POST", "/calls", body, headers);
    const replay = await requestThroughSpecmatic(app.stubUrl, "POST", "/calls", body, headers);
  ...
  it("control headers: query parameter filters survive through the forward path", async () => {
    ...
    const filtered = await requestThroughSpecmatic(
      app.stubUrl,
      "GET",
      "/leads",
      null,
