/**
 * Cascade Integrity: Cross-Boundary dispatch_commands + Referential Integrity
...
describe('Cascade Integrity (full Specmatic stack)', () => {
  let app: E2eApp;
  ...
  describe('call creation cascade: Call + Lead + Agent graph nodes', () => {
    it('POST /calls creates Call, updates Lead.callIds, and increments Agent.dailyCallCount', async () => {
      ...
      // Log call 1
      const callRes1 = await requestThroughSpecmatic(app.stubUrl, 'POST', '/calls', {
        leadId,
      ...
      const callRes2 = await requestThroughSpecmatic(app.stubUrl, 'POST', '/calls', {
        leadId,
  ...
  describe('lead conversion saga -> opportunity referential integrity', () => {
    ...
    beforeAll(async () => {
      ...
      oppId = (opps.body as JsonObject[]).find((o) => o['leadId'] === convertedLeadId)![
        'id'
      ] as string;
    });
  ...
  describe('cascade failure atomicity: non-existent agentId', () => {
    ...
    it('POST /calls whose agent cascade target is missing is rejected atomically  no Call, no Lead mutation', async () => {
      ...
      const callIdsBefore = ((await getGraphNode(app.engineUrl, cfLeadId))!['callIds'] as string[])
        .length;
      ...
      const fwdRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/calls', {
        leadId: cfLeadId,
