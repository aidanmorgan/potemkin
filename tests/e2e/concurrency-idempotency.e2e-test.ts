/**
 * Concurrency & Idempotency: ETag chains, idempotency keys, race conditions
...
describe('Concurrency & Idempotency (Specmatic-backed)', () => {
  let app: E2eApp;
  ...
  describe('optimistic concurrency -- multi-step ETag chain', () => {
    ...
    it('create lead -> get ETag v1', async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', {
        companyName: 'ETag Chain Corp',
        contactName: 'EC',
        phone: '+61 0',
        email: 'etag@chain.test',
        source: 'REFERRAL',
      });
    ...
    it('ETag values increase after mutations', async () => {
      ...
      const currentRes = await requestThroughSpecmatic(app.stubUrl, 'GET', `/leads/${leadId}`);
  ...
  describe('idempotency keys -- full lifecycle', () => {
    ...
    it('same key + different body -> 409 conflict', async () => {
      ...
      await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', body1, {
        'idempotency-key': key,
      });

      const res = await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', body2, {
        'idempotency-key': key,
      });
    ...
    it('GET with idempotency key -> key ignored (queries bypass)', async () => {
      ...
      await requestThroughSpecmatic(app.stubUrl, 'GET', '/leads', null, { 'idempotency-key': key });
      const r2 = await requestThroughSpecmatic(app.stubUrl, 'GET', '/leads', null, {
        'idempotency-key': key,
      });
  ...
  describe('concurrent mutations', () => {
    it('5 concurrent calls to same lead -> all callIds appended, no duplicates', async () => {
      ...
      const promises = Array.from({ length: 5 }, () =>
        requestThroughSpecmatic(app.stubUrl, 'POST', '/calls', {
          leadId,
          agentId: AGENT_ID,
          campaignId: CAMPAIGN_ID,
          outcome: 'INTERESTED',
