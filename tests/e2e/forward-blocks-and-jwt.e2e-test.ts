/**
 * Forward blocks (seeds / workflow / overlay) + JWT through the stub.
 *
 ...
 * (via its potemkin.yml). The harness translates the overlay to a Specmatic
 * overlay file and points Specmatic at it, and splices the auth + forward blocks
 * into the plugin's potemkin.yml.
 */
...
describe("Forward blocks + JWT through the stub", () => {
  let app: E2eApp;
  ...
  it("seed with base:contract serves the patched body through the stub", async () => {
    const res = await fetch(`${app.stubUrl}/seed-contract/contract-1`, {
      headers: {
        ...
        authorization: `Bearer ${signHs256(defaultClaims())}`,
      },
    });
