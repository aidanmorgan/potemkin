/**
 * Multi-Sort & Array Operators: validate the two new query operators
 ...
 *   Single-field ?sort=score&order=desc remains supported.
 *
...
describe('Multi-Sort & Array Operators (full Specmatic stack)', () => {
  let app: E2eApp;
  ...
  describe('Multi-field sorting', () => {
    ...
    it('multi-field sort with pagination envelope (?sort=-score,companyName&limit=3)', async () => {
      ...
      // Top 3 by score DESC: BlueSky (80), Delta (70), then a score lead
    ...
    it('single field without `-` or comma (?sort=status) sorts ascending', async () => {
      const res = await requestThroughSpecmatic(
        app.stubUrl,
        'GET',
        '/leads',
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // Single-field sort remains a supported request form.
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  describe('Array contains operator', () => {
    it('?callIds:contains=<callId> returns the lead containing that call', async () => {
      ...
      const queryRes = await requestThroughSpecmatic(
        app.stubUrl,
        'GET',
        '/leads',
        null,
    ...
    it('?callIds:contains isolates per-lead membership across multiple leads', async () => {
      ...
      const leadARes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', {
      ...
      const callARes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/calls', {
        leadId: leadAId,
        agentId: AGENT_ID,
        campaignId: CAMPAIGN_ID,
        outcome: 'INTERESTED',
      ...
      const leadBRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', {
      ...
      const callBRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/calls', {
        leadId: leadBId,
        agentId: AGENT_ID,
        campaignId: CAMPAIGN_ID,
      ...
      const queryA = await requestThroughSpecmatic(
        app.stubUrl,
        'GET',
        '/leads',
        null,
      ...
      const queryB = await requestThroughSpecmatic(
        app.stubUrl,
        'GET',
        '/leads',
        null,
