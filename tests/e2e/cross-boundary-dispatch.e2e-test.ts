/**
 * Cross-Boundary Dispatch: dispatch_commands graph propagation via the
 ...
 *   examples/crm/dsl/call.yaml (dispatch_commands -> Lead + Agent)
 *   examples/crm/dsl/lead.yaml (dispatch_commands -> Campaign)
 *   examples/crm/dsl/opportunity-close.yaml (dispatch_commands -> Agent)
 */
...
describe("Cross-Boundary Dispatch (full Specmatic stack)", () => {
  let app: E2eApp;
  ...
  describe("First-match semantics: appendCallId fires before mutation guards", () => {
    it("secondary dispatch to Lead only triggers appendCallId, not contactLead", async () => {
      ...
      const res = await requestThroughSpecmatic(app.stubUrl, "POST", "/leads", {
        ...
        contactName: "FM",
        phone: "+61 2 9300 0020",
        email: "firstmatch@test.com",
        source: "WEBSITE",
      });
      expect([200, 201]).toContain(res.status);
  ...
  describe("Multi-hop cascade: POST /calls triggers 3 graph nodes in one transaction", () => {
    it("single POST /calls mutates Call + Lead + Agent graph nodes atomically", async () => {
      const leadRes = await requestThroughSpecmatic(app.stubUrl, "POST", "/leads", {
        ...
        contactName: "TN",
        phone: "+61 2 9300 0030",
        email: "threenode@test.com",
        source: "PARTNER",
      });
