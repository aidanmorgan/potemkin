/**
 * Stage 6 Polish Features via full Specmatic stack.
 *
...
describe('Stage 6 Polish Features (Specmatic-backed)', () => {
  let app: E2eApp;
  ...
  describe('Webhook HMAC signing (secret in global.yaml)', () => {
    ...
    it('signature verifies against the body using the YAML secret', async () => {
      ...
      const createRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', {
        companyName: 'HMAC Verify Corp',
        contactName: 'HV',
        phone: '+61 2 9500 0002',
        email: 'hmac-verify@test.com',
        source: 'REFERRAL',
      });
      const leadId = (createRes.body as JsonObject)['id'] as string;
      ...
      const expected =
        'sha256=' + createHmac('sha256', WEBHOOK_SECRET).update(match!.body).digest('hex');
  ...
  describe('Deprecation / Sunset / Link headers (LeadAddNote boundary)', () => {
    it('POST /leads/{id}/notes response includes Deprecation header from YAML', async () => {
      const res = await requestThroughSpecmatic(
        app.stubUrl,
        'POST',
        `/leads/${APEX_LEAD_ID}/notes`,
        {
          text: 'A note for deprecation testing',
          author: 'Tester',
        },
      );
  ...
  describe('Latency injection (LeadAddNote boundary)', () => {
    ...
    it('boundaries without latency config respond quickly', async () => {
      ...
      const elapsed = Date.now() - start;
      expect(res.status).toBe(200);
      // The 50ms latency only applies to LeadAddNote. The positive latency
      // assertion above proves the configured boundary; this guard leaves
      // room for real Specmatic/JVM transport overhead in aggregate runs.
