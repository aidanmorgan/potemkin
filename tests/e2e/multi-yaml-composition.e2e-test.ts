...
//
// Confirms the loader pipeline assembles one working YAML program from many
// independent module files in examples/crm/dsl/ via the glob declared
// in examples/crm/potemkin.yml. Cross-boundary event references
...
describe('Multi-YAML module composition', () => {
  let app: E2eApp;
  ...
  it('all 21 boundary modules contribute to the contract path table', async () => {
    ...
    for (const p of paths) {
      const res = await requestThroughSpecmatic(app.stubUrl, 'GET', p);
  ...
  it('sub-path boundaries share the same StateGraph as their parent boundary', async () => {
    ...
    const read = await requestThroughSpecmatic(app.stubUrl, 'GET', `/leads/${leadId}`);
  ...
  it('cross-boundary event subscription  LeadConverted feeds CampaignDashboard projection', async () => {
    ...
    const create = await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', {
    ...
    const opp = Object.values(entities).find((e) => (e as { stage?: string }).stage !== undefined);
  ...
  it('global.yaml composition: saga config + idempotency live in a separate file', async () => {
    ...
    const first = await requestThroughSpecmatic(app.stubUrl, 'POST', '/calls', callBody, {
      'idempotency-key': key,
    });
    const replay = await requestThroughSpecmatic(app.stubUrl, 'POST', '/calls', callBody, {
      'idempotency-key': key,
    });
  ...
  it('each sub-path module wires its own reducer patches: against the shared aggregate', async () => {
    ...
    const leadId = (created.body as { id: string }).id;
    await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${leadId}/contact`, { notes: 'x' });
    let read = await requestThroughSpecmatic(app.stubUrl, 'GET', `/leads/${leadId}`);
    ...
    await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${leadId}/qualify`, {});
    read = await requestThroughSpecmatic(app.stubUrl, 'GET', `/leads/${leadId}`);
    expect((read.body as { status: string }).status).toBe('QUALIFIED');
    await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${leadId}/disqualify`, {
