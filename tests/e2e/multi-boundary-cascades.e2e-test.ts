/**
 * Multi-Boundary Cascades via full Specmatic stack.
 *
...
describe("Multi-Boundary Cascades (full Specmatic stack)", () => {
  let app: E2eApp;
  ...
  describe("second call from different agent updates both agents independently", () => {
    ...
    it("second call from Bob increments Bob, leaves Alice unchanged", async () => {
      ...
      expect(leadNode!["callIds"] as string[]).toContain(
        (callRes.body as JsonObject)["id"] as string,
      );
  ...
  describe("lead.yaml dispatch_commands -> Campaign (conditional on assignedCampaignId)", () => {
    it("creating a lead WITH assignedCampaignId dispatches to Campaign graph node", async () => {
      ...
      // Use requestThroughSpecmatic directly since assignedCampaignId bypasses OpenAPI validation
      const res = await requestThroughSpecmatic(app.stubUrl, "POST", "/leads", {
