/**
 * X-Potemkin-* control headers (Tiers 1-7) via full Specmatic stack.
 *
 * Drives each tier's headers through the Specmatic + plugin + Node engine
 * stack via the public Specmatic URL. Verifies that headers parsed at the gateway
...
describe('X-Potemkin-* control headers (full Specmatic stack)', () => {
  let app: E2eApp;
  beforeAll(async () => {
  ...
  describe('Tier 2: side effects', () => {
    it('Skip-Dispatch blocks secondary cascades', async () => {
      ...
      const callsBefore = await requestThroughSpecmatic(app.stubUrl, 'GET', '/calls');
      const initialCallCount = Array.isArray(callsBefore.body)
        ? (callsBefore.body as JsonObject[]).length
        : 0;
      ...
      const callsAfter = await requestThroughSpecmatic(app.stubUrl, 'GET', '/calls');
      const finalCallCount = Array.isArray(callsAfter.body)
        ? (callsAfter.body as JsonObject[]).length
        : 0;
  ...
  describe('Tier 3: identity', () => {
    it('Actor override without any auth token (no actor) → 401', async () => {
      ...
      expect((res.body as JsonObject)['code']).toBe('ADMIN_REQUIRED');
    }, 60_000);
  ...
  describe('Tier 4: time travel', () => {
    it('Read-At-Version on unknown entity → 404', async () => {
      const res = await requestThroughSpecmatic(
        app.stubUrl,
        'GET',
        '/leads/00000000-dead-7000-8000-000000000099',
        null,
        { 'x-potemkin-read-at-version': '3' },
      );
    ...
    it('Replay-Event with unknown id → 404', async () => {
      const res = await requestThroughSpecmatic(
        ...
        `/leads/${APEX_LEAD_ID}`,
        null,
        { 'x-potemkin-replay-event': 'evt-does-not-exist' },
      );
      expect(res.status).toBe(404);
  ...
  describe('Tier 6: observability', () => {
    ...
    it('Span-Name is echoed on response', async () => {
      const res = await requestThroughSpecmatic(
        ...
        `/leads/${APEX_LEAD_ID}`,
        null,
        { 'x-potemkin-span-name': 'e2e-test-span' },
      );
      expect(res.status).toBe(200);
