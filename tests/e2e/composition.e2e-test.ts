/**
 * Cross-file composition capstone (Specmatic-backed).
 *
...
import { requestThroughSpecmatic, getAllEntities, getAllEvents } from './_harness/crm-e2e-helpers';
import type { JsonObject, DomainEvent } from './_harness/crm-e2e-helpers';

describe('Cross-file composition: define-once, map-many, fragment reuse, reactions across instances (Specmatic-backed)', () => {
  let app: E2eApp;

  beforeAll(async () => {
    app = await startE2eApp({ fixtureName: 'composition' });
  }, 120_000);
  ...
  describe('AC1  component instantiated twice at distinct contract_paths; instances are independent', () => {
    it('creates a Document at /documents and reads back DRAFT status', async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, 'POST', '/documents', {
        title: 'My Report',
      });
    ...
    it('creates a Draft at /drafts and reads back PENDING status', async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, 'POST', '/drafts', {
        title: 'My Draft',
      });
      expect([200, 201]).toContain(res.status);
    ...
    it('mutating one instance does not affect the other (states are independent)', async () => {
      ...
      const docRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/documents', {
      ...
      const draftRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/drafts', {
        title: 'Independence Test',
      });
      ...
      const patchRes = await requestThroughSpecmatic(app.stubUrl, 'PATCH', `/documents/${docId}`, {
        title: 'Updated Title',
      });
  ...
  describe('AC2  AuditMixin contributes AuditLogged reducer to both Document and Draft instances', () => {
    it('AuditLogged on Document sets /lastActor (mixin reducer projected)', async () => {
      ...
      const createRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/documents', {
      ...
      const auditRes = await requestThroughSpecmatic(
        app.stubUrl,
        'POST',
    ...
    it('AuditLogged on Draft sets /lastActor (mixin reducer projected on second instance)', async () => {
      // Create a Draft.
      const createRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/drafts', {
  ...
  describe('AC3  component reaction fires into Notification boundary from both instances', () => {
    it('creating a Document triggers NotificationCreated on the Notification boundary (Document instance)', async () => {
      ...
      const notifCountBefore = notificationsBefore.filter(
        (e) => e.type === 'NotificationCreated',
      ).length;

      const docRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/documents', {
        title: 'Reaction Test Doc',
      });
      expect([200, 201]).toContain(docRes.status);
      const docId = (docRes.body as JsonObject)['id'] as string;
    ...
    it('Document and Draft reactions both appear in the Notification boundary event log', async () => {
      ...
      const docRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/documents', {
        title: 'Cross-Instance A',
      });
      expect([200, 201]).toContain(docRes.status);
      const docId = (docRes.body as JsonObject)['id'] as string;

      const draftRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/drafts', {
        title: 'Cross-Instance B',
      });
      expect([200, 201]).toContain(draftRes.status);
      const draftId = (draftRes.body as JsonObject)['id'] as string;
      ...
      const draftCreated = events.find(
        (e) => e.type === 'DocumentCreated' && e.aggregateId === draftId,
      );
