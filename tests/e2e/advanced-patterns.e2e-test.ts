/**
 * Advanced Patterns: Saga, schema evolution, RBAC, observability
...
describe("Advanced Patterns (full Specmatic stack)", () => {
  let app: E2eApp;
  ...
  describe("saga / process manager", () => {
    ...
    it("lead conversion triggers saga -> creates Opportunity in graph", async () => {
      ...
      await requestThroughSpecmatic(app.stubUrl, "POST", `/leads/${convertedLeadId}/contact`, {});
      await requestThroughSpecmatic(app.stubUrl, "POST", `/leads/${convertedLeadId}/qualify`, {});
      await requestThroughSpecmatic(app.stubUrl, "POST", `/leads/${convertedLeadId}/convert`, {
        value: 75000,
        probability: 80,
      });
    ...
    it("multiple conversions -> each creates own opportunity (saga isolation)", async () => {
      const lr2 = await requestThroughSpecmatic(app.stubUrl, "POST", "/leads", {
        companyName: "Saga2 Corp",
        contactName: "S2",
        phone: "+61 0",
        email: "saga2@t.com",
        source: "PARTNER",
      ...
      await requestThroughSpecmatic(app.stubUrl, "POST", `/leads/${lid2}/contact`, {});
      await requestThroughSpecmatic(app.stubUrl, "POST", `/leads/${lid2}/qualify`, {});
      await requestThroughSpecmatic(app.stubUrl, "POST", `/leads/${lid2}/convert`, {
        value: 30000,
        probability: 60,
      });
  ...
  describe("schema evolution -- optional field defaults", () => {
    ...
    it("POST /calls with only required fields -> optional fields get defaults", async () => {
      ...
      const res = await requestThroughSpecmatic(app.stubUrl, "POST", "/calls", {
        leadId,
        agentId: AGENT_ID,
        campaignId: CAMPAIGN_ID,
        outcome: "NO_ANSWER",
    ...
    it("GET entity with minimal fields -> optional fields present as null or default", async () => {
      ...
      const getRes = await requestThroughSpecmatic(app.stubUrl, "GET", `/leads/${id}`);
  ...
  describe("RBAC / authorization simulation", () => {
    ...
    it("DNC without auth -> 401, graph unchanged", async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, "POST", `/leads/${rbacLeadId}/dnc`, {
        reason: "test",
      });
