/**
 * Initialization and Query Mapping via full Specmatic stack.
 *
...
describe('Initialization and Query Mapping (full Specmatic stack)', () => {
  let app: E2eApp;
  ...
  describe('DSL query_mapping filters collections via GET with query params', () => {
    ...
    it('agent.yaml query_mapping currentStatus filters agents', async () => {
      const available = await requestThroughSpecmatic(
        app.stubUrl,
        'GET',
        '/agents',
        null,
    ...
    it('unfiltered GET returns all entities in the boundary', async () => {
      const leads = await requestThroughSpecmatic(app.stubUrl, 'GET', '/leads');
      expect(leads.status).toBe(200);
      expect((leads.body as unknown[]).length).toBe(5);

      const campaigns = await requestThroughSpecmatic(app.stubUrl, 'GET', '/campaigns');
      expect(campaigns.status).toBe(200);
      expect((campaigns.body as unknown[]).length).toBe(2);

      const agents = await requestThroughSpecmatic(app.stubUrl, 'GET', '/agents');
  ...
  describe('query_mapping reflects graph state after mutations', () => {
    it('contacting a NEW lead moves it from status=NEW to status=CONTACTED filter', async () => {
      ...
      const createRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', {
        ...
        contactName: 'FM',
        phone: '+61 2 9300 8000',
        email: 'filtermut@test.com',
        source: 'WEBSITE',
      });
    ...
    it('call.yaml query_mapping leadId filters calls by target lead', async () => {
      const leadRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', {
        ...
        contactName: 'CF',
        phone: '+61 2 9300 8001',
        email: 'callfilter@test.com',
        source: 'PARTNER',
      });
      ...
      await requestThroughSpecmatic(app.stubUrl, 'POST', '/calls', {
        leadId,
        agentId: '00000000-0000-7000-8000-000000000004',
        campaignId: '00000000-0000-7000-8000-000000000001',
        outcome: 'NO_ANSWER',
      });
