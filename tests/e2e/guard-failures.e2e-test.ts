/**
 * Guard Failures via full Specmatic stack.
 *
...
describe('Guard Failures (full Specmatic stack)', () => {
  let app: E2eApp;
  ...
  describe('Opportunity guards (opportunity-advance.yaml / opportunity-close.yaml requires)', () => {
    ...
    it('cannot advance a NEGOTIATING opportunity (already at max advance stage)', async () => {
      ...
      const advRes = await requestThroughSpecmatic(
        app.stubUrl,
        'PATCH',
    ...
    it('cannot advance a WON opportunity', async () => {
      ...
      const closeRes = await requestThroughSpecmatic(
        app.stubUrl,
        'PATCH',
        `/opportunities/${oppId}/close`,
        { outcome: 'WON' },
      );
    ...
    it('cannot win a LOST opportunity', async () => {
      ...
      const closeRes = await requestThroughSpecmatic(
        app.stubUrl,
        'PATCH',
        `/opportunities/${secondOppId}/close`,
        {
          outcome: 'LOST',
