 ...
 * public Specmatic URL and asserts the engine validates JWTs end-to-end:
 *
 *   - signature
 ...
 *
 * The simple-auth `Bearer <id>:<scopes>` shortcut is verified in
 * rbac.e2e-test.ts using the default CRM fixture. JWT mode intentionally
 * rejects that non-JWT form.
 */
...
describe('JWT authentication (configurable)', () => {
  let app: E2eApp;
  ...
  beforeAll(async () => {
    app = await startE2eApp({ fixtureName: 'crm-jwt' });
  ...
  it('no Authorization header on a scoped endpoint → 401', async () => {
    const result = await postForward(app.stubUrl, 'POST', `/leads/${leadIds[8]}/dnc`, {
      reason: 'should fail',
    });
