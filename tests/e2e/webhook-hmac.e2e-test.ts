/**
 * Outbound webhooks with HMAC signature (Specmatic-backed).
 *
...
function startReceiver(port: number, sink: ReceivedRequest[]): Promise<Server> {
...
describe("Outbound webhooks with HMAC signature (Specmatic-backed)", () => {
  let app: E2eApp;
  let receiver: Server;
  ...
  beforeAll(async () => {
    ...
    app = await startE2eApp({ fixtureName: "webhook-hmac" });
  }, 120_000);
  ...
  describe("Webhook delivery", () => {
    it("POST /shipments triggers an outbound webhook POST to the receiver", async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, "POST", "/shipments", {
        trackingRef: "TRK-001",
        destination: "Sydney",
      });
  ...
  describe("HMAC signature header (x-potemkin-signature)", () => {
    ...
    it("signature verifies: sha256=HMAC-SHA256(secret, rawBody) matches header value", async () => {
      ...
      // using the canonical runtime webhook signing contract:
      //   HMAC-SHA256(secret, rawBody).digest('hex')  prefixed with "sha256=".
      const expected =
        "sha256=" + createHmac("sha256", WEBHOOK_SECRET).update(hit!.rawBody).digest("hex");
    ...
    it("a wrong secret produces a different signature (receiver can reject forgeries)", async () => {
      ...
      const wrongSig =
        "sha256=" + createHmac("sha256", "wrong-secret").update(hit!.rawBody).digest("hex");
