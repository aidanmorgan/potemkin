/**
 * Header-triggered chaos behaviors via full Specmatic stack.
 *
...
describe("Header-triggered chaos (full Specmatic stack)", () => {
  let app: E2eApp;
  beforeAll(async () => {
  ...
  describe("X-Potemkin-Force-Latency adds latency to the response", () => {
    ...
    it("non-numeric latency value is ignored and the normal response is returned", async () => {
      const start = Date.now();
      const res = await requestThroughSpecmatic(app.stubUrl, "GET", "/leads", null, {
      ...
      const elapsed = Date.now() - start;
      expect(res.status).toBe(200);
      // Specmatic/JVM startup and shared-run transport overhead can exceed a
      // tight sub-second wall-clock bound. Parser/runtime unit tests prove the
      // malformed value is ignored; this E2E assertion guards against a large
      // accidental delay while asserting the real forwarded response.
  ...
  describe("YAML rules can define the response when chaos headers fire", () => {
    it("Force-Status=599 with a matching YAML rule returns the YAML body, not the generic forced-status body", async () => {
      ...
      expect(body["error"]).toBe("FORCED_STATUS_599");
      expect(body["hint"]).toBe("Client should retry after the response is available.");
  ...
  describe("X-Potemkin-Drop-Connection surfaces a synthetic 504 through Specmatic", () => {
