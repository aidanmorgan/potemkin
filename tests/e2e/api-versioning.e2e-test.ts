/**
 * API versioning through Specmatic.
 *
 ...
 *
 * The public versioned requests go to Specmatic. The plugin discovers the prefixed
 * routes and forwards them; the Node engine resolves the version before contract
 * matching and returns the selected version headers.
 */
...
describe('API versioning (Specmatic-backed)', () => {
  let app: E2eApp;
  beforeAll(async () => {
  ...
  async function ver(method: string, path: string, body?: unknown): Promise<VersionedResponse> {
    ...
    const res = await fetch(`${app.stubUrl}${path}`, init);
  ...
  describe('mutations through versioned prefixes', () => {
    it('POST /v1/leads creates a lead and tags it with v1', async () => {
      const res = await ver('POST', '/v1/leads', {
        companyName: 'V1 Routed Corp',
        contactName: 'V1',
        phone: '+61 2 9700 0001',
        email: 'v1@test.com',
        source: 'WEBSITE',
      });
    ...
    it('POST /v2/leads/{id}/contact transitions the lead and tags v2', async () => {
      ...
      const createRes = await ver('POST', '/v2/leads', {
        companyName: 'V2 Routed Corp',
        contactName: 'V2',
        phone: '+61 2 9700 0002',
        email: 'v2@test.com',
        source: 'REFERRAL',
      });
