/**
 * Event Stream Integrity via full Specmatic stack.
 *
...
describe("Event Stream Integrity (full Specmatic stack)", () => {
  let app: E2eApp;
  let leadId: string;
  ...
  beforeAll(async () => {
    ...
    const createRes = await requestThroughSpecmatic(app.stubUrl, "POST", "/leads", {
      companyName: "EventStream E2E Corp",
      contactName: "ES",
      phone: "+61 0",
      email: "es@e2e.test",
      source: "REFERRAL",
  ...
  it("successful command produces events, rejected command produces none", async () => {
    ...
    const qualRes = await requestThroughSpecmatic(
      app.stubUrl,
      "POST",
