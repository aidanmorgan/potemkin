/**
 * HTTP Conditional Requests (RFC 7232) via full Specmatic stack.
 *
...
// APEX_LEAD_ID is seeded via initialization in examples/crm/dsl/lead.yaml.
...
describe("HTTP Conditional Requests (full Specmatic stack)", () => {
  let app: E2eApp;
  beforeAll(async () => {
  ...
  describe("ETag generation", () => {
    ...
    it("mutating an entity advances the ETag returned by subsequent GETs", async () => {
      ...
      const beforeRes = await requestThroughSpecmatic(app.stubUrl, "GET", `/leads/${leadId}`);
      ...
      const mutateRes = await requestThroughSpecmatic(
        app.stubUrl,
        "POST",
      ...
      const afterRes = await requestThroughSpecmatic(app.stubUrl, "GET", `/leads/${leadId}`);
    ...
    it("two consecutive GETs of the same untouched entity return the same ETag", async () => {
      const first = await requestThroughSpecmatic(app.stubUrl, "GET", `/leads/${APEX_LEAD_ID}`);
      const second = await requestThroughSpecmatic(app.stubUrl, "GET", `/leads/${APEX_LEAD_ID}`);
    ...
    it("different entities have independent ETag values", async () => {
      const apex = await requestThroughSpecmatic(app.stubUrl, "GET", `/leads/${APEX_LEAD_ID}`);
      const bluesky = await requestThroughSpecmatic(
        app.stubUrl,
        "GET",
        `/leads/${BLUESKY_LEAD_ID}`,
      );
  ...
  describe("Last-Modified header", () => {
    it("GET /leads/{id} on a mutated entity includes Last-Modified in HTTP-date format", async () => {
      ...
      const getRes = await requestThroughSpecmatic(app.stubUrl, "GET", `/leads/${leadId}`);
    ...
    it("Last-Modified value matches the entity updatedAt converted to UTC string", async () => {
      ...
      const getRes = await requestThroughSpecmatic(app.stubUrl, "GET", `/leads/${APEX_LEAD_ID}`);
      expect(getRes.status).toBe(200);
  ...
  describe("If-None-Match conditional GETs", () => {
    it("matching If-None-Match returns 304 Not Modified with empty body", async () => {
      const initial = await requestThroughSpecmatic(app.stubUrl, "GET", `/leads/${APEX_LEAD_ID}`);
      expect(initial.status).toBe(200);
      const etag = initial.headers["etag"];
      expect(etag).toBeDefined();

      const conditional = await requestThroughSpecmatic(
        app.stubUrl,
        "GET",
        `/leads/${APEX_LEAD_ID}`,
        null,
    ...
    it("stale If-None-Match after a mutation returns 200 with the new ETag", async () => {
      ...
      const beforeMutation = await requestThroughSpecmatic(app.stubUrl, "GET", `/leads/${leadId}`);
  ...
  describe("HEAD requests inherit conditional semantics", () => {
    it("HEAD /leads/{id} returns 200 with ETag and Last-Modified and empty body", async () => {
      ...
      const res = await requestThroughSpecmatic(app.stubUrl, "HEAD", `/leads/${APEX_LEAD_ID}`);
      expect(res.status).toBe(200);
    ...
    it("HEAD /leads/{id} with matching If-None-Match returns 304 with empty body", async () => {
      const initial = await requestThroughSpecmatic(app.stubUrl, "HEAD", `/leads/${APEX_LEAD_ID}`);
      expect(initial.status).toBe(200);
      const etag = initial.headers["etag"];
      expect(etag).toBeDefined();
