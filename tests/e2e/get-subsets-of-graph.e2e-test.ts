...
import { requestThroughSpecmatic, getEntityCount, adminReset } from './_harness/crm-e2e-helpers';
...
describe('GET returns different subsets of the in-memory graph', () => {
  let app: E2eApp;
  ...
  async function buildGraph(seeds: readonly LeadSeed[]): Promise<readonly string[]> {
    ...
    for (let i = 0; i < seeds.length; i++) {
      ...
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${id}/contact`, { notes: 'auto' });
      ...
      if (seed.lifecycle === 'converted') {
        await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${id}/qualify`, {});
        await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${id}/convert`, {
          value: 50000,
      ...
      if (seed.lifecycle === 'disqualified') {
        await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${id}/disqualify`, {
          reason: 'no fit',
        });
        continue;
      }
      if (seed.lifecycle === 'dnc') {
        await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${id}/dnc`, {
          reason: 'requested',
        });
        continue;
      }
  ...
  it('basic GET /{id} returns the projected state of a single aggregate', async () => {
    ...
    const res = await requestThroughSpecmatic(app.stubUrl, 'GET', `/leads/${id}`);
  ...
  it('basic GET / returns every entity in the boundary', async () => {
    await buildGraph([
      ...
      {
        companyName: 'List Three',
        source: 'REFERRAL',
        campaignId: ALT_CAMPAIGN_ID,
  ...
  it('?status=X returns only entities in that status', async () => {
    ...
    const onlyQualified = await requestThroughSpecmatic(
      app.stubUrl,
      'GET',
      '/leads',
      null,
  ...
  it('GET /calls?leadId=X scopes the result to one lead', async () => {
    ...
    const onLeadA = await requestThroughSpecmatic(
      app.stubUrl,
      'GET',
      '/calls',
      null,
  ...
  it('GET /calls?agentId=X scopes the result to one agent', async () => {
    ...
    const onAgent = await requestThroughSpecmatic(
      app.stubUrl,
      'GET',
      '/calls',
      null,
  ...
  it('?limit=N triggers the pagination envelope; envelope reports totalCount across the full set', async () => {
    ...
    const page = await requestThroughSpecmatic(
      app.stubUrl,
      'GET',
      '/leads',
      null,
  ...
  it('?limit + ?offset returns a precise slice of the same underlying graph', async () => {
    ...
    const full = await requestThroughSpecmatic(app.stubUrl, 'GET', '/leads');
    const slice = await requestThroughSpecmatic(
      app.stubUrl,
      'GET',
      '/leads',
      null,
  ...
  it('multiple GETs against the same graph leave the entity count unchanged', async () => {
    ...
    for (const path of ['/leads', '/campaigns', '/agents', '/calls', '/opportunities']) {
      await requestThroughSpecmatic(app.stubUrl, 'GET', path);
    }
  ...
  it('GET /opportunities (sub-graph) shows entities created by the LeadConversionSaga dispatch', async () => {
    ...
    const res = await requestThroughSpecmatic(app.stubUrl, 'GET', '/opportunities');
  ...
  it('GET /calls returns the call list separately from /leads', async () => {
    ...
    const res = await requestThroughSpecmatic(app.stubUrl, 'GET', '/calls');
