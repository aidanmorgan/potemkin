/**
 * Nested Object Graphs + Request/Response Shape Mismatches
 ...
 *   examples/crm/dsl/opportunity-add-line-item.yaml
 *   examples/crm/dsl/call-add-transcript.yaml
 */
...
describe('Nested Graph Shape Mismatches (full Specmatic stack)', () => {
  let app: E2eApp;
  ...
  describe('Lead Notes: simple request -> complex nested graph node', () => {
    ...
    it('POST { text, author } produces enriched note object in graph with id + createdAt', async () => {
      ...
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${leadId}/notes`, {
  ...
  describe('Opportunity Line Items: request has no total, graph computes it', () => {
    ...
    beforeAll(async () => {
      ...
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${leadId}/contact`, {});
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${leadId}/qualify`, {});
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${leadId}/convert`, {
        value: 100000,
        probability: 80,
      });
    ...
    it('POST { description, quantity, unitPrice } produces line item with computed total + id', async () => {
      ...
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/opportunities/${oppId}/line-items`, {
    ...
    it('guards prevent adding line items to closed opportunities', async () => {
      ...
      const leadRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', {
        companyName: 'Closed Opp Corp',
        contactName: 'CO',
        phone: '+61 2 8000 0011',
        email: 'closed@opp.test',
        source: 'REFERRAL',
      });
      ...
      const opps = await requestThroughSpecmatic(app.stubUrl, 'GET', '/opportunities');
      const closedOppId = (opps.body as JsonObject[]).find((o) => o['leadId'] === lid)![
        'id'
      ] as string;

      // Close the opportunity via PATCH
      await requestThroughSpecmatic(app.stubUrl, 'PATCH', `/opportunities/${closedOppId}/close`, {
        outcome: 'LOST',
      ...
      const res = await requestThroughSpecmatic(
        app.stubUrl,
        'POST',
        `/opportunities/${closedOppId}/line-items`,
        {
  ...
  describe('Call Transcript: batch request -> individually enriched graph entries', () => {
    ...
    beforeAll(async () => {
      const leadRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', {
        ...
        contactName: 'TC',
        phone: '+61 2 8000 0020',
        email: 'transcript@nested.test',
        source: 'WEBSITE',
      });
      const leadId = (leadRes.body as JsonObject)['id'] as string;
    ...
    it('POST { speaker, text } produces enriched entry with timestamp + sequenceNum in graph', async () => {
      ...
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/calls/${callId}/transcript`, {
        speaker: 'Agent',
        text: 'Hello, thank you for taking my call.',
      });
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/calls/${callId}/transcript`, {
        speaker: 'Lead',
      ...
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/calls/${callId}/transcript`, {
        speaker: 'Agent',
        text: 'I am calling about our enterprise solution.',
      });
    ...
    it('subsequent transcript entries continue the sequence numbering', async () => {
      ...
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/calls/${callId}/transcript`, {
        speaker: 'Agent',
        text: 'We offer three tiers of service...',
      });
  ...
  describe('Complex interconnected graph: lead with notes references calls with transcripts', () => {
    ...
    it('build a complex interrelated graph via sequential API calls', async () => {
      ...
      // Log first call
      const c1Res = await requestThroughSpecmatic(app.stubUrl, 'POST', '/calls', {
        leadId,
        agentId: AGENT_ID,
        campaignId: CAMPAIGN_ID,
        outcome: 'INTERESTED',
      ...
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/calls/${call1Id}/transcript`, {
        speaker: 'Agent',
      ...
      // Log second call
      const c2Res = await requestThroughSpecmatic(app.stubUrl, 'POST', '/calls', {
        leadId,
        agentId: AGENT_ID,
        campaignId: CAMPAIGN_ID,
        outcome: 'INTERESTED',
      ...
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/calls/${call2Id}/transcript`, {
        speaker: 'Agent',
        text: 'Following up on pricing discussion',
      });
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/calls/${call2Id}/transcript`, {
        speaker: 'Lead',
        text: 'We want to proceed with tier 2',
      });
      await requestThroughSpecmatic(app.stubUrl, 'POST', `/calls/${call2Id}/transcript`, {
        speaker: 'Agent',
        text: 'Excellent, I will prepare the proposal',
      });
