/**
 * identity.key: aggregate id extracted from various request sources.
 *
 ...
 * Transport: Specmatic JVM → Kotlin plugin → Node engine.
 */
...
describe('identity.key: all non-header sources (Specmatic-backed)', () => {
  let app: E2eApp;

  beforeAll(async () => {
    app = await startE2eApp({ fixtureName: 'identity-key' });
  }, 60_000);
  ...
  describe('POST /tokens with X-Token-Id header', () => {
    ...
    it('GET /tokens/<explicit-id> retrieves the entity by the header-derived key', async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, 'GET', `/tokens/${EXPLICIT_TOKEN_ID}`);
  ...
  describe('Two POSTs with different X-Token-Id values create independent entities', () => {
    ...
    it('GET /tokens/tok_independent_a returns the correct entity (not B)', async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, 'GET', `/tokens/${ID_A}`);
      expect(res.status).toBe(200);
      const body = res.body as JsonObject;
    ...
    it('GET /tokens/tok_independent_b returns the correct entity (not A)', async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, 'GET', `/tokens/${ID_B}`);
      expect(res.status).toBe(200);
      const body = res.body as JsonObject;
  ...
  describe('identity.key.from: path  aggregate id from a named path parameter', () => {
    ...
    it('two POSTs with different ref values create independent aggregates', async () => {
      ...
      const resA = await requestThroughSpecmatic(app.stubUrl, 'POST', `/orders/${refA}`, {
        product: 'Gadget',
        quantity: 1,
      });
      ...
      const resB = await requestThroughSpecmatic(app.stubUrl, 'POST', `/orders/${refB}`, {
        product: 'Gizmo',
        quantity: 2,
      });
      ...
      const getA = await requestThroughSpecmatic(app.stubUrl, 'GET', `/orders/${refA}`);
      expect(getA.status).toBe(200);
      expect((getA.body as JsonObject)['product']).toBe('Gadget');

      const getB = await requestThroughSpecmatic(app.stubUrl, 'GET', `/orders/${refB}`);
  ...
  describe('identity.key.from: query  aggregate id from a query parameter', () => {
    ...
    it('two POSTs with different sessionId values create independent aggregates', async () => {
      ...
      const resA = await requestThroughSpecmatic(
        app.stubUrl,
        'POST',
        '/session-events',
        { eventType: 'LOGIN', userId: 'user_charlie' },
        {},
        { sessionId: sessA },
      );
      expect(resA.status).toBe(201);
      ...
      const resB = await requestThroughSpecmatic(
        app.stubUrl,
        'POST',
        '/session-events',
        { eventType: 'LOGOUT', userId: 'user_diana' },
        {},
        { sessionId: sessB },
      );
      expect(resB.status).toBe(201);
      ...
      const getA = await requestThroughSpecmatic(app.stubUrl, 'GET', `/session-events/${sessA}`);
      expect(getA.status).toBe(200);
      expect((getA.body as JsonObject)['userId']).toBe('user_charlie');

      const getB = await requestThroughSpecmatic(app.stubUrl, 'GET', `/session-events/${sessB}`);
      expect(getB.status).toBe(200);
  ...
  describe('identity.key.from: payload  aggregate id from a request body field', () => {
    it('POST /subscriptions with body.accountId returns 201 and body.id equals accountId', async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, 'POST', '/subscriptions', {
        accountId: EXPLICIT_ACCOUNT_ID,
        plan: 'premium',
      });
      expect(res.status).toBe(201);
      const body = res.body as JsonObject;
    ...
    it('two POSTs with different accountId values create independent aggregates', async () => {
      ...
      const resA = await requestThroughSpecmatic(app.stubUrl, 'POST', '/subscriptions', {
        accountId: accA,
        plan: 'starter',
      });
      expect(resA.status).toBe(201);
      ...
      const resB = await requestThroughSpecmatic(app.stubUrl, 'POST', '/subscriptions', {
        accountId: accB,
        plan: 'enterprise',
      });
      expect(resB.status).toBe(201);
      ...
      const getA = await requestThroughSpecmatic(app.stubUrl, 'GET', `/subscriptions/${accA}`);
      expect(getA.status).toBe(200);
      expect((getA.body as JsonObject)['plan']).toBe('starter');

      const getB = await requestThroughSpecmatic(app.stubUrl, 'GET', `/subscriptions/${accB}`);
      expect(getB.status).toBe(200);
