/**
 * Query Extensions: verifies three additions to the query engine end-to-end
...
describe("Query Extensions (full Specmatic stack)", () => {
  let app: E2eApp;
  ...
  describe("Field selection (sparse fieldsets)", () => {
    ...
    it("GET /leads (no fields param) returns full entities by default", async () => {
  ...
  describe("Cursor-based pagination", () => {
    ...
    it("GET /leads?limit=2&cursor=<from previous page> returns the NEXT 2 items (no overlap)", async () => {
      ...
      const page1 = await requestThroughSpecmatic(
        app.stubUrl,
        "GET",
        "/leads",
        null,
      ...
      const page2 = await requestThroughSpecmatic(
        app.stubUrl,
        "GET",
        "/leads",
        null,
  ...
  describe("Deep nested filtering", () => {
    it("simple field equality via queryMapping filters the expected seeded entities", async () => {
  ...
  describe("Cursor sweep covers all seeded leads", () => {
    it("walking pages with cursor visits every seeded lead exactly once", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const res = await requestThroughSpecmatic(app.stubUrl, "GET", "/leads", null, {}, query);
        expect(res.status).toBe(200);
        const env = res.body as JsonObject;
        const items = env["items"] as JsonObject[];
