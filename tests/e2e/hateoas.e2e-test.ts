/**
 * HATEOAS / hypermedia links via full Specmatic stack.
 *
...
interface WithLinks extends JsonObject {
  _links: Record<string, HateoasLink>;
}

describe('HATEOAS hypermedia links (full Specmatic stack)', () => {
  let app: E2eApp;
  ...
  describe('state-dependent action links', () => {
    ...
    it('CONTACTED lead surfaces qualify and disqualify links (not contact)', async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, 'GET', `/leads/${BLUESKY_LEAD_ID}`);
      const body = res.body as WithLinks;
    ...
    it('QUALIFIED lead surfaces convert and disqualify links', async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, 'GET', `/leads/${CORNER_LEAD_ID}`);
      const body = res.body as WithLinks;
    ...
    it('DISQUALIFIED lead surfaces self + dnc but no transition actions', async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, 'GET', `/leads/${DELTA_LEAD_ID}`);
      const body = res.body as WithLinks;
  ...
  describe('state transitions reshape the available links', () => {
    it('contacting a NEW lead replaces contact with qualify/disqualify', async () => {
      ...
      const createRes = await requestThroughSpecmatic(app.stubUrl, 'POST', '/leads', {
        companyName: 'HATEOAS Transition Corp',
        contactName: 'HT',
        phone: '+61 2 9100 4001',
        email: 'hateoas-trans@test.com',
        source: 'WEBSITE',
      });
      expect([200, 201]).toContain(createRes.status);
      const leadId = (createRes.body as JsonObject)['id'] as string;

      const before = await requestThroughSpecmatic(app.stubUrl, 'GET', `/leads/${leadId}`);
      ...
      const after = await requestThroughSpecmatic(app.stubUrl, 'GET', `/leads/${leadId}`);
