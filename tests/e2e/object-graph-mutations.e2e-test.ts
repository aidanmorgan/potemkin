/**
 * DSL Object Graph Mutations via full Specmatic stack.
 *
...
describe("DSL Object Graph Mutations (full Specmatic stack)", () => {
  let app: E2eApp;
  ...
  describe("Lead creation: DSL event_catalog + reducer builds graph node", () => {
    ...
    it("POST /leads creates entity with fields from DSL reducer", async () => {
      const res = await requestThroughSpecmatic(app.stubUrl, "POST", "/leads", {
        companyName: "E2E Graph Corp",
        contactName: "Graph User",
        phone: "+61 2 9000 1234",
        email: "graph@e2e.test",
        source: "REFERRAL",
      });
      ...
      expect(node!["score"]).toBe(80); // REFERRAL → 80 via CEL score expression
  ...
  describe("DSL script CEL score expression maps sources to scores", () => {
    it("each source produces correct score in graph", async () => {
      ...
      for (const { source, expected } of sources) {
        const res = await requestThroughSpecmatic(app.stubUrl, "POST", "/leads", {
          companyName: `Score ${source}`,
          contactName: "S",
          phone: "+61 0",
          email: `s-${source.toLowerCase()}@e2e.test`,
          source,
        });
