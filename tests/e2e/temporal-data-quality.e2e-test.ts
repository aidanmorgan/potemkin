/**
 * Temporal Data Quality: Temporal Coherence + Boundary Values
 ...
 *   examples/crm/dsl/lead.yaml
 *   examples/crm/dsl/lead-contact.yaml
 *   examples/crm/dsl/lead-add-note.yaml
 *   examples/crm/dsl/opportunity.yaml
 *   examples/crm/dsl/opportunity-add-line-item.yaml
 *   examples/crm/dsl/call.yaml
 *   examples/crm/dsl/call-add-transcript.yaml
 *   examples/crm/dsl/agent.yaml
 *   examples/crm/dsl/campaign.yaml
 */
...
describe('Temporal Data Quality (full Specmatic stack)', () => {
  let app: E2eApp;
  ...
  it('sequential notes on a lead have increasing createdAt', async () => {
    ...
    await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${id}/notes`, {
      text: 'First note',
      author: 'Agent A',
    });
  ...
  it('agent with dailyCallQuota:1 is accepted (minimum valid boundary)', async () => {
    const res = await requestThroughSpecmatic(app.stubUrl, 'POST', '/agents', {
  ...
  it('converting a lead with value:0 is rejected by the opportunity positive-value guard', async () => {
    ...
    const oppsBeforeRes = await requestThroughSpecmatic(app.stubUrl, 'GET', '/opportunities');
    ...
    await requestThroughSpecmatic(app.stubUrl, 'POST', `/leads/${leadId}/convert`, { value: 0 });

    // The guard rejected the opportunity, so no Opportunity exists for this lead
    // and the total opportunity count is unchanged.
    const oppsAfterRes = await requestThroughSpecmatic(app.stubUrl, 'GET', '/opportunities');
