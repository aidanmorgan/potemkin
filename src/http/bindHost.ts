...
export function resolveBindHost(surface: BindSurface, opts: ResolveBindHostOptions = {}): string {
  ...
  const errorCode =
    surface === "dsl" ? "BOOT_ERR_REMOTE_DSL_NOT_ALLOWED" : "BOOT_ERR_REMOTE_STATE_NOT_ALLOWED";
  ...
  if (allow !== "1") {
    throw new BootError(errorCode, `${overrideKey}=${override} requires ${allowKey}=1`, {
      overrideKey,
      allowKey,
      surface,
    });
