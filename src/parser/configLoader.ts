// Reads potemkin.yml from disk, resolves module globs, and produces the
// source-independent YAML compilation input. The top-level
...
export interface LoadedConfig {
  ...
  /** Typed configuration metadata for RuntimeSystem. */
  readonly configuration: PotemkinConfiguration;
  /** Raw YAML modules for the source-independent runtime compiler. */
  ...
  readonly watchIgnores: readonly string[];
  // Note: the `seeds`/`workflow`/`overlay`/`governance` forward blocks from
  // potemkin.yml are intentionally NOT surfaced here. The Kotlin Specmatic
  // plugin parses them directly from potemkin.yml (ForwardBlocks.parse in
...
export async function loadPotemkinConfig(
  ...
  //
  // The three-way split introduced by C1 also accepts files without
  ...
  const yamlProgram: YamlProgramInput = {
    modules: compileModules,
    ...(globalYaml === undefined ? {} : { globalYaml }),
    ...(compileComponentModules.length === 0 ? {} : { componentModules: compileComponentModules }),
    ...(compileUseMappingModules.length === 0
      ? {}
      : { useMappingModules: compileUseMappingModules }),
  };
  ...
  return {
    ...
    watchGlobs: [
      ...config.modules.map((pattern) => path.resolve(configDir, pattern)),
      ...(config.openapi?.map((pattern) => path.resolve(configDir, pattern)) ?? []),
      ...(config.typescript?.scan.flatMap((entry) =>
        entry.include.map((pattern) => path.resolve(configDir, pattern)),
      ) ?? []),
    ],
    watchIgnores:
      config.typescript?.scan.flatMap((entry) =>
        (entry.exclude ?? []).map((pattern) => path.resolve(configDir, pattern)),
      ) ?? [],
  };
}

async function resolveModuleGlobs(patterns: readonly string[], cwd: string): Promise<string[]> {
 ...
 * back to a YAML string. compileYaml re-parses this single string via
