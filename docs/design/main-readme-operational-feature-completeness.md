...
selectors; `G-15` is closed by the complete first-match, wildcard, and
forwarding matrix.
...
caller receives the response. `G-22` is closed by the forwarded patch-journal
combinations; the validation-controls parity
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `POST /_admin/reset`         | Reset volatile state to the deterministic seed baseline. Clear event log, projections, sessions, idempotency, dynamic faults, and virtual clock.                                  | New engine reset code clears these stores; `runtime-ttl` proves event/state/idempotency/session/fault/clock clearing for YAML and TypeScript; `runtime-derived-projection` proves seeded projection restoration; `session-authoring-parity` proves an active session is invalidated and the clock returns to a known offset through Specmatic for YAML, TypeScript, and mixed configurations. | `G-23` is closed by the fresh-boot comparison and reset/admin matrix.                                   |
...
- `G-09` is closed: `authoring-parity` proves declarative-fault precedence over an
  ...
  `G-10` and `G-12` are covered by the direct/forwarded jitter and
  connection-drop matrix. `session-authoring-parity`
...
- `G-20` is closed. The `runtime-observability` Specmatic matrix proves the
...
- `G-21` is closed: `validation-controls-authoring-parity` proves request and
  ...
  Specmatic for YAML, TypeScript, and mixed loading.
- `G-22` is closed: response-mask ordering is implemented and unit-tested. The
  ...
  caller-facing response is returned. The canonical matrix also covers the
  pagination and root-shape combinations; the multi-operation journal is
  forwarded and removed from caller-facing responses.
...
Latest verification (2026-08-05): the retained Jest baseline passes 212 suites
and 2,813 tests. The targeted Specmatic parity matrix passes 44 suites and 475
tests across YAML, TypeScript, and mixed loading; the observability suite is
